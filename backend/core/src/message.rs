...
/// Tauri event as soon as it is polled from the broker, so the Data tab can
/// paint rows as they arrive instead of staying empty until the whole fetch
/// finishes. `request_id` is generated by the frontend and echoed back here
/// so a tab that has since started another fetch (or been stopped) can
/// discard a superseded request's late-arriving events.
///
